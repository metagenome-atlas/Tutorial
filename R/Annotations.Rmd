...
keggmodules_file = paste0(atlas_wd_folder,"annotations/dram/kegg_modules.tsv")
```

# Taxonomy

```{r, read_tax}
Tax <- read_tsv(taxonomy_file)
kable(Tax)
```
...
genome_quality <- read_tsv(quality_file) %>%
...
read_stats <-read_tsv(readstats_file) %>% filter(Step == "QC" )
...
d <- tibble( Sample= colnames(Counts)[-1],
  ...
  mutate(Mapping_rate = Mapped_Reads / Total_Reads *100)
...
moudle_names= kegg_modules %>% select(c('module','module_name')) %>% distinct()
