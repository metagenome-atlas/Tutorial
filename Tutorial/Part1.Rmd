...
In the first part of the tutorial, you will 1.
learn how to install Atlas, 2.
init a new project and 3.
run it.
...
Make shure that the `terminal`is selected in the bottom plane. You can make it bigger. 
...
Run `atlas --version` and make sure you have the latest version installed.
![Version](https://anaconda.org/bioconda/metagenome-atlas/badges/version.svg)
...
Samples should be alphanumeric names.
...
Adapt the contaminant_references as follows:
    ...
    contaminant_references:
      ...
      host: /oath/to/databases/host_genome.fasta
...
This allows atlas to submitt each step to the cluster while specifying the number of cores and memory, not bad!
...
➤ If the qc  workflow is not finished. rpess `CTR + C.` 
...
➤ Copy it to your computer and open it with a web browser
...
Once the assembly workflow is finished try to find the quality-controlled reads (.fastq) for sample1?
...
There are many different binner tools, as explained in the class. 
